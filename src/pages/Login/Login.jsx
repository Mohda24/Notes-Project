...
function Login() {
    const {loading,login,userName}=useAuth();
    const ref=useRef();
        ...
        <main className='min-h-screen flex items-center justify-center'>
            ...
            className={`login bg-primary-color w-96 rounded-md shadow-lg p-8 opacity-0 translate-y-4 ${isVisible && "animate-fadeIn"}`}>
                ...
                className='mt-10 flex flex-col items-center gap-[20px]'>
                    ...
                    className='w-full bg-transparent border border-[#ffffff27] py-2 px-6 rounded placeholder:text-[#ffffff4e]' type="text" placeholder="Your Cin" />
                    <input
                    value={user.password}
                    onChange={(e)=>setUser({...user,password:e.target.value})}
                    className='w-full bg-transparent border border-[#ffffff27] py-2 px-6 rounded placeholder:text-[#ffffff4e]' type="password" placeholder="Your Password" />
                    <button
